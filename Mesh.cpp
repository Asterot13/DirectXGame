...
Mesh::Mesh(const wchar_t* full_path) : Resource(full_path)
{
	...
	std::vector<VertexrMesh> list_vertices;
		...
		for (size_t s = 0; s < shapes.size(); s++)
		{
			...
			for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++)
			{
				...
				for (unsigned char v = 0; v < num_face_verts; v++)
				{
					...
					VertexrMesh vertex(Vector3D(vx, vy, vz), Vector2D(tx, ty), Vector3D(nx, ny, nz));
	...
	m_vertex_buffer = GraphicsEngine::get()->getRenderSystem()->createVertexBuffer(&list_vertices[0], sizeof(VertexrMesh), (UINT)list_vertices.size(), shader_byte_code, (UINT)size_shader);
...
size_t Mesh::getNumMaterialsSlots()
{
