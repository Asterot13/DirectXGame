...
void SwapChain::resize(unsigned int width, unsigned int height)
{
	...
	m_swap_chain->ResizeBuffers(1, width, height, DXGI_FORMAT_R8G8B8A8_UNORM, 0); //DXGI_FORMAT_D24_UNORM_S8_UINT
	reloadBuffers(width, height);
}
...
void SwapChain::reloadBuffers(unsigned int width, unsigned int height)
{
	...
	if (FAILED(res))
	{
		throw std::exception("Swap chain buffer not created successfully");
	}
