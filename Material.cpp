...
Material::Material(const wchar_t* vertex_shader_path, const wchar_t* pixel_shader_path)
{
	...
	if (!m_vertex_shader) throw std::exception("Vertex shader not created successfully");
	...
	if (!m_pixel_shader) throw std::exception("Pixel shader not created successfully");
}
...
void Material::setData(void* data, unsigned int size)
{
	if (!m_constant_buffer)
		...
		m_constant_buffer->update(GraphicsEngine::get()->getRenderSystem()->getImmidiateDeviceContext(), data);
}
