...
void PostProcessingDemo::render()
{
	...
	GraphicsEngine::get()->getRenderSystem()->getImmidiateDeviceContext()->clearDepthStencil(this->m_depth_stencil);
	GraphicsEngine::get()->getRenderSystem()->getImmidiateDeviceContext()->setRenderTarget(this->m_render_target, this->m_depth_stencil);
...
void PostProcessingDemo::updateModel(Vector3D position, Vector3D rotation, Vector3D scale, const std::vector<MaterialPtr>& list_material)
{
	constant cc;
...
void PostProcessingDemo::onMouseMove(const Point& mouse_pos)
{
	...
	m_delta_mouse_x = (float)(mouse_pos.m_x - (int)(wins_size.left + (width / 2.0f)));
	m_delta_mouse_y = (float)(mouse_pos.m_y - (int)(wins_size.top + (height / 2.0f)));
